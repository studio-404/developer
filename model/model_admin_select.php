...
class model_admin_select extends connection{
	...
	public function select_admins($c){
		...
		if(isset($_GET['search']) && !empty($_GET['search']) ){
			$sql = 'SELECT * FROM `studio404_users` WHERE `status`!=:status AND (`namelname` LIKE :namelname OR `username` LIKE :namelname OR `ucode` LIKE :namelname) ORDER BY `id` DESC';
			$search = '%'.$_GET['search'].'%';
			$exe_array = array(":status"=>1,":namelname"=>$search);
		}else{
			$sql = 'SELECT * FROM `studio404_users` WHERE `status`!=:status ORDER BY `id` DESC';
